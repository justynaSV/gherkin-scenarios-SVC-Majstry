...
If the input above is a Jira issue URL (e.g. `https://<site>.atlassian.net/browse/<KEY>`) or a bare issue key (e.g. `PROJ-123`), use configured Atlassian MCP tools to fetch that epic/story instead of asking the user to paste it:
...
First output assumptions when needed, then the suggested path as a comment, then the full `.feature` file content. After the code block, ask a single plain-text question: `Should I create and save this file at <features/path>/<kebab-case-name>.feature? (yes/no)` (append the create:module note to the question text if a new module folder is created, and note that `npm run trace:generate` will also run to update the traceability matrix).
