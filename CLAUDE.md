...
- Maintainer note (only applies if sibling repos are actually present, e.g. in the maintainer's multi-root
  ...
  should be mirrored across all 4 unless a change is explicitly repo-specific (e.g. actual `.feature` content,
  module folders). Do not reference or try to access sibling repos when they aren't present in the current workspace.
